use crate::{config::LspConfig, languages::get_language, Document};
...
macro_rules! replace_with_capitalization {
    ($text:expr, $($placeholder:expr => $value:expr),*) => {{
        ...
        $(
            ...
            result = result.replace(concat!("{", $placeholder, "}"), $value).replace(concat!("{", $placeholder, ":u}"), &capitalized);
        )*
        result
    }};
}
...
impl<'a> Placeholders<'a> {
    pub fn new(doc: Option<&'a Document>, config: &'a LspConfig, workspace: &'a str) -> Self {
